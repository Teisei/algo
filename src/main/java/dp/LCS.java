...
public class LCS<T extends Comparable<? super T>> {
    ...
    @Test
    public void testOne() {
        int[] a = {0, 0, 1, 2, 3, 4, 5, 6, 7, 7, 8, 9};
        int[] b = {0, 1, 3, 5, 7, 7, 9};
        Person[] personsa = new Person[a.length];
        Person[] personsb = new Person[b.length];
        for (int i = 0; i < a.length; i++) {
            personsa[i] = new Person("" + a[i]);
        }
        for (int i = 0; i < b.length; i++) {
            personsb[i] = new Person("" + b[i]);
        }
        LCS<Person> lcs = new LCS<Person>();
        lcs.getLCS2(personsa, personsb);
        lcs.getLCS(personsa, personsb);
