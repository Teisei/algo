...
public class HashTrieTree<T> implements Comparable<HashTrieTree<T>> {
    ...
    * */
    ...
    public static void main(String args[]) {
        HashTrieTree<Person> trietree = new HashTrieTree<Person>();

        String word = "照顾";
        List<Person> infos = new ArrayList<Person>();
        infos.add(new Person("take care of ab"));
        infos.add(new Person("kill somebody"));


        String word1 = "慷慨";
        List<Person> infos1 = new ArrayList<Person>();
        infos1.add(new Person("good"));
        infos1.add(new Person("kill somebody"));

        trietree.add(word, infos);
        trietree.add(word1, infos1);

        List<Person> res = trietree.search(word);
        for (Person p : res) {
            System.out.println(p.toString());
        }


        res = trietree.search(word1);
        for (Person p : res) {
            System.out.println(p.toString());
        }
